        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
					...
					@endif
				    <form action="{{ route('tiket.checkIn'); }}" method="POST" class="mb-4">
				        @csrf
				    ...
				    <div class="alert alert-success mb-4" role="alert">
					  	<span class="block">ID Tiket ditemukan, berikut biodata pemilik :</span>
					  	<span class="block">Nama : {{ $pemesan->name }}</span>
					  	<span class="block">NIK : {{ $pemesan->nik }}</span>
					  	<span class="block">Email : {{ $pemesan->email }}</span>
					</div>
				</div>
            </div>
        </div>
